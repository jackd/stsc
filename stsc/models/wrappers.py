...
def get_inputs(max_events: int, batch_size: int, grid_shape: tp.Iterable[int]):
    ...
    batch_splits = keras.Input(batch_shape=(batch_size + 1,), dtype="int32")
...
def per_event_model(
    ...
    **compile_kwargs,
    ...
    if reduction == "mean":
        example_features = segment_ops.segment_sum(
            event_features, batch_ids, num_segments=batch_size, indices_are_sorted=True
    ...
    # if normalize:
    #     stream = stream.masked_batch_norm(momentum=0.9)
    # # stream = stream.map_features(keras.layers.Dropout(dropout_rate))
    # # stream = stream.exclusive_conv(num_classes, stream.stream.grid_shape)
    # stream = stream.get_exclusive_patches(
    #     stream.stream.grid_shape,
    #     sample_rate=1,
    #     decay_rate_initializer=keras.initializers.RandomNormal(stddev=1.0),
    #     decay_rate_activation=functools.partial(
    #         real_decay_rate_activation_v2, epsilon=1e-2
    #     ),
    # )
    # stream = stream.map_features(keras.layers.Dropout(dropout_rate))
    # stream = stream.map_features(keras.layers.Dense(num_classes))
    ...
    # batch_lengths = stream.stream.batch_lengths
    # event_features = stream.compute_features()
    # example_features = stream.stream.pool_features(event_features, reduction=reduction)
    ...
    preprocessor, model = split(
        ...
        preprocessor_outputs=(batch_ids, batch_lengths),
    )
    ...
    *model_inputs, batch_ids, batch_lengths = preprocessor.outputs
    dtype = example_sample_weight.dtype
    ...
    event_sample_weight = ops.take(
        ops.pad(
            ops.where(
                ...
                / (
                    ops.cast(valid_batch_count * batch_lengths, dtype)
